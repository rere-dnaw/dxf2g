...
class Breaks(object):
    ...
    """
    def __init__(self, layer_contents):
        self.layer_contents = layer_contents
        """
        Process layer_contents: Each non-BREAKS: layers shapes are checked against all the shapes in all BREAKS: layers.
        If a shape is intersected twice by a break-shape, the shape will be broken.
        """
        self.breakLayers = []
        for layerContent in self.layer_contents.break_layer_iter():
