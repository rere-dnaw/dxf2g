...
class MyPostProcessor(object):
    ...
    def __init__(self, script_dir):
        ...
        self.loadCreateConfigFiles(script_dir)

    def loadCreateConfigFiles(self, script_dir):
        """
        ...
        try:
            lfiles = sorted(os.listdir(os.path.join(script_dir + '\\config')))
            """
            FIXME Folder needs to be empty or valid config file within.
            """
            # logger.debug(lfiles)
        except ValueError:
            logger.debug("config file missing!!")
        ...
        if len(self.postprocessor_files) == 0:
            logger.debug("config file missing11!!")#added by RW
    ...
    def getPostProVars(self, file_index):
        ...
        self.vars.
        """
        PostProConfig = MyPostProConfig(file_name=self.postprocessor_files[file_index])
    ...
    def write_gcode_be(self, load_filename):
        ...
        type g-code in addition the dxf- file_name, dxf2gcode version etc. is
        added to the code. Otherwise just the defined text in the PostProcessor
        file is added.
        @param load_filename: The name of the original dxf file_name to append
